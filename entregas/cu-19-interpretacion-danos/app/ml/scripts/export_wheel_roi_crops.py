# script completamente vibecodeado para obtener los crops de las ruedas
# para usarlas en el tip adapter usando CLIP
# REUTILIZAR ESTO PARA LOS FAROS
...
DEFAULT_WHEEL_PROMPTS = [
    "a visible car tire",
    "a car wheel",
    "a vehicle wheel",
]

TIRE_FLAT_CATEGORY_ID = 6
...
def load_runtime_dependencies() -> None:
    ...
    try:
        ...
        raise SystemExit(
            "Missing Python dependency for wheel ROI export: "
            f"{exc.name}. Run this script from the ML/Jupyter environment "
            "that has numpy, torch, transformers, pillow, pycocotools and tqdm."
        ) from exc

    # The local sam3 conda env has scipy/sklearn/accelerate installed but broken
    # for the optional generation/loss paths that transformers imports eagerly.
    # SAM3 inference here does not need those optional backends.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Export wheel ROI crops for a small flat/healthy support set. "
            "Flat crops use COCO tire-flat GT boxes by default; healthy crops "
            "use SAM3 wheel proposals."
        )
    )
    parser.add_argument(
        "--ann-path",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_root / "ml" / "notebooks" / "results" / "wheel_roi_crops",
    )
    parser.add_argument("--flat-ids", nargs="+", type=int, default=FLAT_IMAGE_IDS)
    parser.add_argument("--healthy-ids", nargs="+", type=int, default=HEALTHY_IMAGE_IDS)
    parser.add_argument(
        "--flat-source",
        choices=["gt", "sam3"],
        default="gt",
        help="Use tire-flat GT boxes for flat examples, or SAM3 wheel proposals.",
    )
    parser.add_argument(
        "--wheel-prompts",
        nargs="+",
        default=DEFAULT_WHEEL_PROMPTS,
        help="Prompts used by SAM3 to find wheel/tire ROIs.",
    )
    ...
    parser.add_argument("--max-mask-area-frac", type=float, default=0.95)
...
def xywh_to_xyxy(bbox_xywh: list[float] | tuple[float, float, float, float]) -> tuple[float, float, float, float]:
...
def build_crop_name(
    *,
    ...
    return f"{image_id:06d}_{label}_{source}_{crop_index:02d}_{suffix}.jpg"


def export_gt_flat_crops(
    *,
    coco: COCO,
    image_id: int,
    img_dir: Path,
    ...
    ann_ids = coco.getAnnIds(imgIds=[image_id], catIds=[TIRE_FLAT_CATEGORY_ID])
    anns = coco.loadAnns(ann_ids)
    if not anns:
        raise ValueError(f"No tire-flat GT annotation found for image_id={image_id}")
    ...
    for crop_index, ann in enumerate(anns):
        ...
        crop_path = output_dir / "flat" / crop_name
...
def export_sam3_crops(
    *,
    ...
    info, image_path, image = load_image(coco, image_id, img_dir)
    proposals = run_sam3_wheel_proposals(
        model=model,
        processor=processor,
        image=image,
        prompts=prompts,
    ...
    for crop_index, proposal in enumerate(proposals):
        ...
        crop_name = build_crop_name(
            ...
            source="sam3",
            crop_index=crop_index,
            suffix=f"score{proposal.score:.3f}".replace(".", "p"),
        )
    ...
    if not rows:
        print(f"[warn] No SAM3 wheel crops exported for image_id={image_id} ({label}).")
...
def main() -> None:
    ...
    needs_sam3 = bool(args.healthy_ids) or args.flat_source == "sam3"
    model = None
    processor = None
    if needs_sam3:
    ...
    print("Exporting flat wheel crops...")
    for image_id in tqdm(args.flat_ids, desc="flat"):
        if args.flat_source == "gt":
            rows.extend(
                export_gt_flat_crops(
                    coco=coco,
                    image_id=image_id,
    ...
    print("Exporting healthy wheel crops...")
    for image_id in tqdm(args.healthy_ids, desc="healthy"):
        assert model is not None and processor is not None
        rows.extend(
            export_sam3_crops(
                coco=coco,
    ...
    flat_count = sum(1 for row in rows if row["label"] == "flat")
    healthy_count = sum(1 for row in rows if row["label"] == "healthy")
    print(f"Done. flat={flat_count}, healthy={healthy_count}")
